...
MANUAL_REVIEW_PROMPTS = [
    {
        "name": "jp_chat",
        "system": "Reply in Japanese. Keep it warm, direct, and concise. Do not reveal hidden reasoning.",
        "user": "こんにちは。あなたの強みを日本語で短く自己紹介して。",
    },
    {
        "name": "structured_summary",
        "system": "Return strict JSON only.",
        "user": "theme を vram_efficiency、focus を preserve quality にして JSON を返して。",
    },
]

PERFORMANCE_PROMPT = {
    "system": "Reply in Japanese. Keep it short. Do not reveal hidden reasoning.",
    "user": "このモデルの現在の雰囲気を一言で説明して。",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare multiple local NVFP4 models under identical runtime settings.")
    parser.add_argument(
        "--models",
        nargs="*",
        default=[],
        help="Model directory names under the nvfp4 root. Defaults to all model directories there.",
    )
    parser.add_argument("--gateway-host", default="127.0.0.1")
    parser.add_argument("--gateway-port", type=int, default=8000)
    parser.add_argument("--timeout", type=int, default=900, help="Timeout in seconds per vLLM reconfigure.")
    parser.add_argument("--keep-last", action="store_true", help="Keep the last tested model active instead of restoring.")
...
def validate_canary(name: str, text: str) -> tuple[bool, str]:
    ...
    if name == "translation_en":
        ok = "quality" in lowered and "vram" in lowered and any(token in lowered for token in ("reduce", "lower", "decrease", "preserve", "maintain"))
...
def discover_models(names: list[str]) -> list[Path]:
    if names:
...
def build_markdown_report(report: dict[str, Any]) -> str:
    lines = [
        ...
        "## Summary",
        "",
        "| model | idle VRAM MB | peak VRAM MB | TTFT ms | tok/s | canaries |",
        "| --- | ---: | ---: | ---: | ---: | --- |",
    ]
    ...
    for item in report["results"]:
        ...
        for prompt in item["manual_review"]:
            lines.extend(
                [
                    f"- `{prompt['name']}`",
...
def main() -> int:
    ...
    json_path = OUT_DIR / f"model-matrix-probe-{timestamp}.json"
    md_path = OUT_DIR / f"model-matrix-probe-{timestamp}.md"

    report: dict[str, Any] = {
        "generated_at": dt.datetime.now(dt.timezone.utc).isoformat(),
        "runtime": dict(RUNTIME_UPDATES),
        "results": [],
    ...
    try:
        for model_path in models:
            ...
            updates["MODEL_PATH"] = str(model_path)
            updates["SERVED_MODEL_NAME"] = model_name
            write_env_updates(ENV_PATH, updates)
            recreate_vllm()
            status, _ = wait_for_runtime(
                args.gateway_host,
                args.gateway_port,
                str(model_path),
                int(RUNTIME_UPDATES["MAX_MODEL_LEN"]),
                args.timeout,
            )
            ...
            manual_review = []
            for case in MANUAL_REVIEW_PROMPTS:
                request_id, output_text = run_non_stream(
                    args.gateway_host,
                    args.gateway_port,
                    model_name,
